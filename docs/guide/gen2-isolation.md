...
### Gen 2: Hard Isolation (`SubInterpreterEnv`)
...
1. It attempts to load the module in the sub-interpreter via `interp.try_import("module_name")`.
2. If CPython rejects it (`ImportError: ... does not support loading in subinterpreters`), EnvKnit catches the error and returns `False`.
